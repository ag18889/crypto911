...
<head>
    <meta charset="UTF-8">
    <title>交易所资产总览截图生成器 V17.0 (最终无瑕疵版)</title>
    <style>
        /* 颜色定义 */
        :root {
            --text-color: #333; /* 柔和深灰 */
        ...
        #mock-app {
            ...
            color: var(--text-color); /* App 主体文字颜色 */
            position: relative;
            display: flex;
        ...
        /* 核心内容区 */
        .content-main { flex-grow: 1; padding: 15px 15px; background-color: #ffffff; overflow-y: auto; }
        
        /* 导航 */
        .main-nav { display: flex; gap: 20px; font-size: 18px; font-weight: 700; padding-bottom: 20px; }
        .main-nav-item { color: #999; cursor: pointer; font-weight: 600;}
        .main-nav-item.active { color: var(--soft-black); font-weight: 700; position: relative; } /* 活跃项更黑更粗 */
        ...
        .asset-summary h1 { font-size: 32px; font-weight: 700; margin: 0 0 5px 0; }
        
        /* 盈亏样式调整 */
        .profit-loss-summary {
            font-size: 15px;
            color: var(--text-color); /* 修正：保证此区域默认文字颜色是黑色的 */
            gap: 5px;
            margin-top: 5px;
            font-weight: 500;
        }
        ...
        /* 操作按钮组 CSS (保持 V16.0 的对标宽度) */
        .action-buttons { display: flex; gap: 10px; margin-bottom: 40px; }
        ...
        /* 资产列表区 (保持不变) */
        .asset-list-header { display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 1px solid #f0f2f5; font-size: 15px; font-weight: 600; margin-bottom: 10px; }
        .asset-list-item { display: flex; justify-content: space-between; padding: 10px 0; font-size: 16px; border-bottom: 1px solid #f0f2f5; }
        .asset-list-label { color: var(--text-color); }
        ...
        #calculated-result .error { color: var(--red); font-weight: bold; }
        #calculated-result .warning { color: var(--binance-yellow); font-weight: bold; }
    </style>
</head>
<body>
    ...
    <div id="container">
        <div id="editor">
            <h3>📱 手机样式</h3>
            <label for="select-phone-style">手机样式</label>
            <select id="select-phone-style">
                <option value="notch">苹果刘海屏 (iPhone)</option>
                <option value="standard-bar" selected>标准圆角 (Android)</option>
            </select>
            
            <label for="input-custom-time">自定义时间 (如 15:58)</label>
            <input type="text" id="input-custom-time" value="15:58">
            
            <hr style="border-color: #555; margin: 20px 0;">

            <h3>💰 资产数据编辑</h3>
            <label for="input-total-asset">预估总资产 ($)</label>
            <input type="text" id="input-total-asset" value="2250870.69">

            <label for="input-pl-amount">今日盈亏金额 ($)</label>
            <input type="text" id="input-pl-amount" value="115780.017">

            <label for="select-pl-color">今日盈亏状态</label>
            <select id="select-pl-color">
                <option value="green">盈利 (绿)</option>
                <option value="red">亏损 (红)</option>
            </select>

            <label for="input-spot">现货余额 ($)</label>
            <input type="text" id="input-spot" value="893718.69">
            
            <label for="input-funding">资金余额 ($)</label>
            <input type="text" id="input-funding" value="1298810.05">

            <div id="calculated-result">合约余额 (自动计算): <span>$58,341.95</span></div>
            
            <label for="input-future">合约余额 ($) - 自动计算</label>
            <input type="text" id="input-future" value="58341.95" readonly>


            <hr style="border-color: #555; margin: 20px 0;">
            <button id="save-button">一键保存为 PNG 图片</button>
        </div>
        ...
        <div id="mock-app">
            ...
            <div class="content-main">
                <div class="main-nav">
                    <span class="main-nav-item active">总览</span>
                    <span class="main-nav-item">现货</span>
                    <span class="main-nav-item">资金</span>
                    <span class="main-nav-item">合约</span>
                </div>

                <div class="asset-summary">
                    <h3>预估总资产 <i id="eye-icon" class="fas fa-eye"></i></h3>
                    <h1 id="total-asset-output">$2,250,870.69</h1>
                    <p class="profit-loss-summary" id="pl-summary-output">
                        今日盈亏 <span id="pl-value-output" class="pl-value green">+$115,780.017 (5.14%)</span>
                    </p>
                </div>

                <div class="action-buttons">
                    <span class="action-btn">添加资金</span>
                    <span class="action-btn secondary">转出</span>
                    <span class="action-btn secondary">划转</span>
                </div>
            ...
            <div class="nav-bar">
                <div class="nav-item">
                    <i class="fas fa-wallet nav-icon"></i> <span>首页</span>
                </div>
                <div class="nav-item">
                    <i class="fas fa-chart-line nav-icon"></i> <span>行情</span>
                </div>
                <div class="nav-item">
                    <i class="fas fa-exchange-alt nav-icon"></i> <span>交易</span>
                </div>
                <div class="nav-item">
                    <i class="fas fa-file-contract nav-icon"></i> <span>合约</span>
                </div>
                <div class="nav-item active">
                    <i class="fas fa-user nav-icon"></i> <span>资产</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const plSummaryOutput = document.getElementById('pl-summary-output');
        const plValueOutput = document.getElementById('pl-value-output'); // 新增的盈亏值 span
        ...
        function updateDisplay() {
            ...
            if (totalAssetRaw < MIN_TOTAL_ASSET) {
                assetWarning = `<span class="warning">⚠️ 警告: 总资产低于 $${formatNumber(MIN_TOTAL_ASSET)}，已强制显示 $${formatNumber(MIN_TOTAL_ASSET)}。</span>`;
            ...
            const plSign = plColorSelect.value === 'red' ? '-' : '+'; // App 界面正数也带 + 号
            ...
            if (futureBalance < 0) {
                 calculationMessage = `<span class="error">❌ 错误: 现货+资金超过总资产 ($${formatNumber(Math.abs(futureBalance))})。请调整。</span>`;
                 futureOutput.textContent = '$0.00'; 
            } else {
                 futureOutput.textContent = `$${formatNumber(futureBalance)}`;
                 calculationMessage = `合约余额 (自动计算): <span>$${formatNumber(futureBalance)}</span>`;
            }
        ...
        function saveScreenshot() {
            ...
            html2canvas(mockApp, {
                ...
                saveButton.textContent = '一键保存为 PNG 图片';
            }).catch(err => {
                 saveButton.disabled = false;
                 saveButton.textContent = '生成失败，请截图';
